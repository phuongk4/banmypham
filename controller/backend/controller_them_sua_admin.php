<?php 
	class controller_them_sua_admin{
		public $model;
		public function __construct(){
			...
			switch($act){
				case "edit":
					...
					$arr = $this->model->get_a_record("select * from tbl_admin where pk_user_id=$id");
				break;
				case "do_edit":
					...
					$this->model->execute("update tbl_admin set c_fullname='$c_fullname' where pk_user_id=$id");
					...
					if($c_password != ""){
						...
						//update ban ghi co id truyen vao
						$this->model->execute("update tbl_admin set c_password='$c_password' where pk_user_id=$id");
					}
				...
				case "do_add":
					...
					$c_password = md5($c_password);
					//insert ban ghi
					$this->model->execute("insert into tbl_admin(c_fullname,c_email,c_password) values('$c_fullname','$c_email','$c_password')");
					//quay tro lai trang user
					echo '<META HTTP-EQUIV="Refresh" Content="0; URL=admin.php?controller=user">';
					// header("location:admin.php?controller=user");
				break;
