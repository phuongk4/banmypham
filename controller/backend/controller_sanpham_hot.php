		...
		public function __construct(){
			...
			switch($act){
				case "delete":
					...
					$this->model->execute("delete from tbl_sanpham_hot where id_sp=$id");
					//quay lai trang tin tuc
					echo '<META HTTP-EQUIV="Refresh" Content="0; URL=admin.php?controller=product_hot">';
					//-----------
				break;
			}
			//------------
			...
			$total = $this->model->get_num_rows("select c_name from tbl_sanpham_hot");
			...
			//------------
			$arr = $this->model->get_all("select * from tbl_sanpham_hot order by id_sp desc limit $from,$record_per_page");
			//load view
			include "view/backend/view_product_hot.php";
			//------------
		}
	}
