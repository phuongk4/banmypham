...
<input type="text" name="c_price" value="<?php echo isset($arr->c_price)?$arr->c_price:""; ?>" required class="form-control">
	...
	<?php 
		//lay toan bo ban ghi trong tbl_loaisp
		$category = $this->model->get_all("select * from tbl_loaisp");
		foreach($category as $rows)
		{
	 ?>
<option <?php if(isset($arr->fk_category_product_id)&&$arr->fk_category_product_id==$rows->id_loaisp){ echo "selected"; } ?> value="<?php echo $rows->id_loaisp; ?>"><?php echo $rows->c_name; ?></option>
	<?php } ?>
	</select>
							</div>
						</div>
